    ...
    decreases INC_LEVELS - subtree.level,
{
    ...
    if subtree.value.is_node() {
        if meta_addr(subtree.value.node.unwrap().slot_index) == excepted_addr {
...
impl<'rcu, C: PageTableConfig> CursorOwner<'rcu, C> {
            ...
            self.push_level_owner_spec(guard).metaregion_sound(regions),
    {
        ...
        let cur_entry_addr = meta_addr(cur_entry.node.unwrap().slot_index);
        ...
        assert forall |i: int|
            ...
            if cont_i.guard.inner.inner@.ptr.addr()
                == guard.inner.inner@.ptr.addr()
            {
                let addr = meta_addr(cont_i.entry_own.node.unwrap().slot_index);
                assert(addr == meta_addr(cur_entry.node.unwrap().slot_index));
            ...
            self.pop_level_owner_spec().0.metaregion_sound(regions),
    {
        ...
        let child_addr = meta_addr(child.entry_own.node.unwrap().slot_index);
